...
On start-up, Sync Gateway will generate the mobile-specific metadata for all the pre-existing documents in the Couchbase Server bucket. From then on, documents can be inserted on the Server directly (with {SQLpp} or SDKs) or through the Sync Gateway REST API.
...
They are also accessible from {SQLpp} in Couchbase Server 5.5 or above with the `().xattrs` property. For example, `SELECT meta().xattrs._sync from travel-sample where Meta().id = &quot;user::demo&quot;;`.

*WARNING:* The sync metadata is maintained internally by Sync Gateway and its structure can change at any time. It should not be used to drive business logic of applications. The direct use of the {SQLpp} query is unsupported and must not be used in production environments.
...
__optional__|The maximum wait time, in milliseconds,for {SQLpp} or View queries made by Sync Gateway

Log warnings if the run time of a {SQLpp} or View query, made by Sync Gateway, exceeds this value.|integer
|**sync** +
...
__optional__|The view query timeout in seconds. This property allows you to specify the time Sync Gateway should wait for a view query response from Couchbase Server before it times out. The timeout is used for both view and {SQLpp} queries issued by Sync Gateway.|integer
|===
